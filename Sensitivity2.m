function [shapeSens] = Sensitivity2( Be,C,struc,U,edofMat,depsilon,ScanLSgridPhi,Dtheta_dphi_i_minus_1,Dtheta_dphi_i_plus_1,Dtheta_dphi_j_minus_1,Dtheta_dphi_j_plus_1)
...
ele = 0;
epsilon = 1.5;  % band width along the edge of phi = 0
...
for elz = 1:nelz
    for ely = 1:nely
        for elx = 1:nelx
            ...
            if ScanLSgridPhi( ely,elx, elz) < -epsilon
                Coeff = 0;
            elseif ScanLSgridPhi( ely, elx, elz) > epsilon
                Coeff = 0;
            else
                Coeff = 1/(2*epsilon) + 1/(2*epsilon)*cos( pi*ScanLSgridPhi(ely,elx,elz)/epsilon);
            end
            
            shapeSens(ely,elx,elz) = Coeff*( term1 + term2 + term3 + term4 );
        end
    end
end
